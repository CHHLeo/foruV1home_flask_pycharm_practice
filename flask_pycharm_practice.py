...
# user_name = 'root'
# password = ''
# databast_host_address = 'localhost:3306'
# database_name = 'test'
user_name = 'ch2leo'
password = '21070527'
databast_host_address = 'ch2leo.mysql.pythonanywhere-services.com'
database_name = 'ch2leo$test'
# app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqldb://' + user_name + ':' + password + '@' + \
#                                         databast_host_address + '/' + database_name
# db.create_all()
...
# CollinsSession = scoped_session(sessionmaker(bind=colllins_engine))
...
def voc_database(data):
    ...
    def update_wanted_dic(remember="1"):
        for i in request.form:
            ...
            pos = request.form[i]
            # print voc, pos
            datas = db.get_engine(app, bind='Collins').execute(text('UPDATE vocabulary SET rememver = "' +
                                                                    remember + '" WHERE lower(voc) = "' +
                                                                    voc.lower() + '"'))
            if pos != 'on':
                datas = db.get_engine(app, bind='Coca').execute(
